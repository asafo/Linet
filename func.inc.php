...
function updateSql($cond,$array,$tablename){
	$tablename=sqlText($tablename);
	//not very officint but works need to mod some day
	foreach ($array as &$value) $value="'".sqlText($value)."'";
	if (isset($array['password'])) $array['password']="PASSWORD(".$array['password'].")";
	foreach ($cond as &$value) $value="'".sqlText($value)."'";
	if (isset($cond['password'])) $cond['password']="PASSWORD(".$cond['password'].")";
	...
	$con.=array_shift($dnoc)."=".array_shift($cond).");";
	$beforelast=count($array)-1;
	$yarra=array_keys($array);
	$data='';
	print $beforelast;
	for ($i=0; $i<$beforelast; ++$i){
		$data.=array_shift($yarra)."=".array_shift($array).", ";
	}
	$data.=array_shift($yarra)."=".array_shift($array);
...
function selectSql($cond,$tablename,$fields=NULL){
	...
	$tablename=sqlText($tablename);
	//same prablom as upstiers need som work
	foreach ($cond as &$value) $value="'".sqlText($value)."'";
	if (isset($cond['password'])) $cond['password']="PASSWORD(".$cond['password'].")";
...
function deleteSql($cond,$tablename){
	//same prablom as upstiers need som work
	foreach ($cond as &$value) $value="'".sqlText($value)."'";
	if (isset($cond['password'])) $array['password']="PASSWORD(".$array['password'].")";
