...
if($step > 0) {
	...
	if($doctype == 0) {
		...
		if($doctype != DOC_RECEIPT) {
			...
			if(empty($customer))
				$query = "SELECT * FROM $receiptstbl WHERE ";
			else
				$query = "SELECT * FROM $receiptstbl WHERE account='$customer' AND ";
			$query .= "issue_date>='$begindate' AND issue_date<='$enddate'";
			$query .= " AND prefix='$prefix' ORDER BY refnum DESC";
		}
...
//			print "DocNum: $docnum<br>\n";
			if($doctype > DOC_RECEIPT) {	/* invoice and receipt together */
...
//			print "<input type=\"button\" onclick=\"window.location.href='$url'\"";
//			print "value=\"שלח בדואר אלקטרוני\">\n";
			print "</td></tr>\n";
		}
