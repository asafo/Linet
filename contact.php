...
if($action == 'edit') {
	...
	$query = "SELECT * FROM $receiptstbl WHERE account='$num' AND prefix='$prefix' ";
	$query .= "ORDER BY issue_date DESC";
	$result = DoQuery($query, __LINE__);
	if(mysql_num_rows($result)) {
		while($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
...
$search = _("Search");
$srchform = <<<EOF1
<form action="?module=contact&amp;action=search" method="post">
...
$tblheader = <<<EOT
<input type="button" value="$l1" onClick="window.location.href='?module=contact&amp;action=add&amp;type=$t1'">
<input type="button" value="$l2" onClick="window.location.href='?module=contact&amp;action=add&amp;type=$t2'">
<br>
...
print "<br>\n";
print "<div class=\"righthalf1\">\n";
$l = _("Customers/Suppliers managmenet");
print "<h3>$l</h3>\n";
print "$srchform";
print "</div>\n";
print "<div class=\"lefthalf1\">\n";
...
$t1 = CUSTOMER;
$t2 = SUPPLIER;
$query = "SELECT * FROM $accountstbl WHERE prefix='$prefix' AND type='$t1' ";
$query1 = '';
if($company)
