...
function ReceiptsDetails($bkmvdata, $bkrecnum, $regnum, $docnum) {
	...
	$query = "SELECT issue_date FROM $receiptstbl WHERE prefix='$prefix' AND refnum='$docnum'";
...
function ExportReceipts($bkmvdata, $bkrecnum, $regnum, $begindate, $enddate) {
	...
	$query = "SELECT * FROM $receiptstbl WHERE prefix='$prefix' ";
...
<script type="text/javascript">
	 ...
	 */
	$query = "SELECT regnum,vat,companyname FROM $companiestbl WHERE prefix='$prefix'";
	$f = __FILE__;
	$l = __LINE__;
...
*/
	...
	fwrite($fd, "<br>$l<br>\n");
//	fwrite($fd, "<br>ביצוע ממשק פתוח הסתיים בהצלחה<br>\n");
