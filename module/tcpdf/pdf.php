...
function title ($label, $total, $position)
{
	...
	$mid = $y1 + ($y2 / 2);
	$width=70;
	$this->SetFillColor(3200);//192
	$this->RoundedRect($r1-16, $y1-1, $width, $y2+1, 2.5, 'DF');
	$this->SetXY( $r1 -16 , $y1 );
...
function tableWrapper($position)
{
	...
	$mid = $y1 + (13 / 2);
	$width=10;
	$this->RoundedRect($r1, $y1, ($r2 - $r1), $y2, 4.5, 'B');//adam: D
	$this->Line( $r1+1, $mid+1, $r2+1, $mid+1);
	/*$this->SetXY( $r2 + ($r1-$r2)/2 - 3, $y1+3 );
	$this->SetXY( $r2 + ($r1-$r2)/2 - 5, $y1 + 9 );*/
...
function addBubbleBlock ($page, $title, $position)
{
	...
	//$this->SetFont( "BitstreamVeraSans-Roman", "B", 10);
	$this->SetFont("Helvetica", "", 10);
	$this->Cell($width,5, $title, 0, 0, "C");
	$this->SetXY( $r1 + ($r2-$r1)/2 - 5, $y1 + 9 );
	$this->SetFont( "Helvetica", "", 10);
			...
			break;
	  		...
	  		case 'Price':
				if($taxtype == "individual")
					$this->Line( $colX, $y1, $colX, ($y1+$y2));//adam: -37
				else
					$this->Line( $colX, $y1, $colX, ($y1+$y2));//adam: -43
			break;
	  		default:
...
function addProductLine( $line, $tab, $totals='' )
{
	...
	//adam: very dumb $this->Text(60,20,count($tab));
	while ( list( $lib, $pos ) = each ($columns) )
	//while (count($tab)
	{
		$longCell = $pos -2;
