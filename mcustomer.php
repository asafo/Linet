<?PHP
/*
 | monthly customer report for Freelance accounting system
...
if(!isset($module)) {
	...
	$reptitle = "הכנסות לפי לקוח וחודש";
...
function GetCustomerIncome($acct, $begin, $end) {
...
print "<br><h1>הכנסות לפי לקוח וחודש</h1>\n";
...
if($filerep) {
	...
	print "<td style=\"width:8em\">לקוח&nbsp;</td>\n";
	foreach($montharr as $m)
		print "<td style=\"width:4em\">$m</td>\n";
	print "<td>סה\"כ</td>\n";
	print "</tr><tr class=\"tblhead\">\n";
	print "<td colspan=\"14\">הכנסות</td>\n";
	print "</tr>\n";
}
$t = CUSTOMER;
...
while($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
	...
	for($i = 0; $i < 12; $i++) {
		...
		$edate = "$y-$m-$l";
		$t = round(GetCustomerIncome($num, $bdate, $edate), 0);
		$total[$i] = $t;
		$sum += $t;
	}
	if($sum == 0)
...
if(isset($module) && !$filerep) {
	...
	print "&nbsp;&nbsp;";
	print "<input type=\"button\" value=\"יצוא לקובץ\" onclick=\"window.location.href='?module=mcustomer&file=1'\">\n";
	print "</div>\n";
}
if($filerep) {
	fclose($fd);
	...
	print "<a href=\"$filename\">mcustomer.csv</a></h2>\n";
