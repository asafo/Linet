...
global $prefix, $accountstbl, $companiestbl, $transactionstbl, $docstbl;
...
//	$text.= "Query: $query<br>\n";
	...
	$query = "SELECT * FROM $receiptstbl WHERE issue_date='$mdate' AND prefix='$prefix'";
	$result = DoQuery($query, "NumEvents");
	$events += mysql_num_rows($result);
...
function FindEvents($d, $m, $y) {
	...
	$begindate = "1-1-$y";
	$enddate = "31-12-$y";
//	$text.= "<br><h1>אירועים לתאריך: $d-$m-$y</h1>\n";
...
//					$text.= "סכום: $sum </td>\n";
	...
	while($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		NewRow();
		$text.= "<td>$DocTypeStr <a href=\"$text.=doc.php?win=1&amp;doctype=$type&amp;docnum=$docnum&amp;prefix=$prefix\">$docnum</a> ";
		$l = _("To customer");
...
//		$text.= "סכום: $sum </td>\n";
	...
	while($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		$sum = $line['sum'];
		$refnum = $line['refnum'];
		$q = "SELECT company FROM $receiptstbl WHERE refnum='$refnum'";
...
if($action == 'showevents') {
	...
	$text.= "<br>\n";
	$text.= "<form><input type=\"button\" value=\"חזור\" onclick=\"history.back()\"></form>\n";
}
else {
	$text.= $calendar_html;
	$n = NumEvents($d, $m, $y);
